 ...
 */
export async function downloadAudio(url, dir, onProgress, caps = {}) {
  const args = [
    '--no-playlist',
    '--no-warnings',
    '--no-part',
  ...
  // Reject oversized material before a single byte is transferred, rather than
  // finding out after it has already crossed a home connection.
  if (caps.maxDurationSeconds > 0) {
    args.push('--match-filter', `duration < ${caps.maxDurationSeconds}`);
  }
  if (caps.maxFilesize) {
    args.push('--max-filesize', caps.maxFilesize);
  }
  ...
  if (!file) {
    // yt-dlp exits cleanly when --match-filter or --max-filesize skips an item,
    // so "no file" here usually means the video was outside the allowed limits.
    const limit = caps.maxDurationSeconds > 0
      ? ` This demo only accepts videos under ${Math.floor(caps.maxDurationSeconds / 60)} minutes — run it locally for anything longer.`
      : '';
    throw new ToolError(`No audio was produced for that link.${limit}`, { tool: YTDLP });
  }
