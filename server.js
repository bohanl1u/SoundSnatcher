...
app.get('/api/health', async (_req, res) => {
  res.json({ ok: true, ytdlp: YTDLP, ffmpeg: FFMPEG });
});

app.post('/api/snatch', async (req, res) => {
  ...
  processJob(job, Boolean(wantAnalysis)).catch((err) => {
    job.stage = 'error';
...
app.get('/api/job/:id/download', async (req, res) => {
  const job = getJob(req.params.id);
  if (!job) {
    res.status(404).json({ error: 'That snatch has expired. Paste the link again.' });
    return;
